...
export default function Layout() {
  ...
  return (
    <div className="h-full w-full max-w-7xl mx-auto flex overflow-hidden">
      {/* Desktop sidebar */}
        ...
        <div className="mt-auto flex items-center gap-3 p-2 rounded-xl hover:bg-white/5">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-medium truncate flex items-center gap-1.5">
              {user?.display_name}
              <RoleBadge role={(user as any)?.role} size={14} />
              <AccountBadge type={(user as any)?.account_type} role={(user as any)?.role} size={13} />
            </div>
            <div className="text-xs text-slate-500 truncate">#{user?.handle}</div>
          </div>
          <button onClick={logout} className="h-8 w-8 grid place-items-center rounded-lg hover:bg-white/10 text-slate-400">
      ...
      <aside className="hidden xl:block w-72 shrink-0 p-4">
        <div className="bg-panel border border-edge rounded-2xl p-4 sticky top-4">
          <div className="flex items-center gap-2 font-semibold mb-3">
            <Sparkles className="h-4 w-4 text-brand" /> Trending tags
          </div>
