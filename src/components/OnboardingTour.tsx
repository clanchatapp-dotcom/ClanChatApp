...
// "Before you enter the clubhouse…" — one-time welcome + Comfort-Zone setup for new sign-ups.
  ...
  const steps = [
    {
      ...
      body: (
        <p className="text-slate-300">
          ClanChat is an online version of chilling with your homies — not a stage, not a broadcast.
        ...
        <div className="flex items-center gap-2">
          ...
          {!last ? (
            ...
            <button onClick={finish} disabled={saving} className="ml-auto px-6 py-2.5 rounded-xl bg-gradient-to-r from-brand to-violet-600 font-semibold disabled:opacity-50 flex items-center gap-2">
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} Enter the clubhouse
            </button>
          )}
        </div>
      </div>
